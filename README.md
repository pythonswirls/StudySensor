...
* The HC-SR04 Sonic Sensor is incredibly unreliable. It is a sensor that returns the time it takes for a pulse to be received back from being bouced off a surface, but the time returned can be varied heavily, leading to inaccurate measurements. A microcontroller compatible IR (Infared) distance sensor could be a more reliable but equally cost-effective alternative.
